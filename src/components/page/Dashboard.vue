        ...
        <div class="wrap-box-item marginRight24" v-if="show_home">
            <div class="top-box">
                <el-main class="box-el-main" v-loading="statusCountLoading">
                    ...
                    <div class="right-box">
                        <div class="count-value" @click="gotoAfterSale">{{ statusCountObj.count_status_apply }}</div>
        ...
        <div class="wrap-box-item" v-if="show_home">
            <div class="sales-statistics-wrap">
                ...
                <el-main v-loading="salesLoading">
                    ...
                    <div ref="saleTrendEChart" class="single-chart"></div>
                </el-main>
            </div>
        </div>
    </div>
...
export default {
    ...
    mounted() {
        ...
        if (!is_admin) {
            ...
            if (
                PermissionsList.indexOf('mall-backend-order-report-index-status') > -1 &&
    ...
    methods: {
        ...
        newUserShopChange() {
            if (this.new_user_shop_id > -1) {
                this.showNewUserData = this.allNewUserData.filter(ev => {
                    return ev.shop_id === this.new_user_shop_id
                })
            } else {
        ...
        tabClick() {
            if (this.tabPosition === '今日') {
                ...
                if (this.shop_id > -1) {
                    this.showSaleData = this.allSalesData.filter(ev => {
                        return ev.shop_id === this.shop_id
                    })
                } else {
                ...
                tooltip: {
                    ...
                    formatter: function(params) {
                        ...
                        for (let i = 0; i < params.length; i++) {
                            let _point =
                                '<span style="width: 6px;height: 6px;border-radius: 100%;display: inline-block;margin: 0 5px 2px 0;background: ' + color_list[i] + '"></span>'
        ...
        getSalesDataToday() {
            queryOrderHour()
                .then(res => {
                    ...
                    if (this.shop_id > -1) {
                        this.showSaleData = allHourData.filter(ev => {
                            return ev.shop_id === this.shop_id
                        })
                    } else {
        ...
        setSalesOptions(n) {
            ...
            if (this.showSaleData.length === 0) {
                this.showSaleData = [
                    {
                        shop_id: -1,
                        daily_report_info: [{ date: '0000-00-00', money: 0, num: 0, order_count: 0 }]
                    }
                ]
            }
            // 至少有一条数据
        ...
        setSalesHoursOptions() {
            ...
            if (this.showSaleData.length === 0) {
                this.showSaleData = [
                    {
                        shop_id: -1,
                        hourly_report_info: [{ date: '00', money: 0, num: 0, order_count: 0 }]
                    }
                ]
            }
            this.showSaleData.forEach((event, index) => {
        ...
        fillSalesOptions(x_axis, _list) {
            ...
            let all_money_now = 0,
                all_num_now = 0,
                all_order_count_now = 0
            let all_money_before = 0,
                all_num_before = 0,
                all_order_count_before = 0
