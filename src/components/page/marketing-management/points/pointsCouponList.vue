        ...
        <el-table :height="tableHeight" :data="list" v-loading.body="listLoading" :header-cell-style="$tableHeaderColor" element-loading-text="Loading" fit>
            <el-table-column label="名称" min-width="300">
                <template slot-scope="scope">
                    <span class="edit-goods" @click="editGoods(scope.row)" v-hasPermission="'mall-backend-points-coupon-detail'">
                        {{ scope.row.title }}
                    </span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="已兑/总数" width="120">
                <template slot-scope="scope">
                    <span>{{ scope.row.salesQty }}/{{ scope.row.quotaQty }}</span>
                </template>
            </el-table-column>
            <el-table-column label="店铺" width="120">
                <template slot-scope="scope">
                    <span>{{ shopList.find(item => item.id == shopId).shop_name }}</span>
                </template>
            </el-table-column>
...
<script>
import { queryPointsCouponList, deletePointsCoupon, putPointsCouponStatus } from '@/api/points'
...
export default {
    ...
    created() {
        bus.$on('refreshPointsCouponList', target => {
            if (target === 'add') {
    ...
    destroyed() {
        bus.$off('refreshPointsCouponList')
    },
    async mounted() {
