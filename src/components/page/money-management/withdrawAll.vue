<template>
    <div class="app-container" @click.stop="searchShow = false">
        ...
        <el-table :height="tableHeight" :data="list" v-loading.body="listLoading" :header-cell-style="$tableHeaderColor" element-loading-text="Loading" fit>
            ...
            <el-table-column label="申请提现金额" width="106">
                <template slot-scope="scope">
                    <span>{{ formatMoney(scope.row.money) }}</span>
                </template>
            </el-table-column>
            <el-table-column label="申请时间" width="178">
                <template slot-scope="scope">
                    <span>{{ scope.row.apply_time }}</span>
                </template>
            </el-table-column>
            <el-table-column label="到账时间" width="178">
                <template slot-scope="scope">
...
export default {
    ...
    methods: {
        ...
        closeSearchItem(item, i) {
            ...
            if(item.label == 'successTime'){
                this.$set(this.formFilter, 'successTime', [])
                this.$set(this.formFilter, 'success_time_lte', '')
                this.$set(this.formFilter, 'success_time_gte', '')
            }
