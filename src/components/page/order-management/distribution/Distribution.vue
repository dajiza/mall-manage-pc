<template>
    <div class="distribution-container" id="containerWrap"> <!--container-wrap-->
            ...
            <div class="handle-box">
                <el-form :model="timeForm" :inline="true" ref="timeForm">
                    ...
                    <div class="btn-right">
                        ...
                        <el-button type="primary" v-hasPermission="'distribution-order-create'" @click="orderProcessCreate()">生成配货单</el-button>
                    </div>
                </el-form>
            </div>
        ...
        <div class="container container-table-has-search m-t-16 p-t-0">
            ...
            >
                ...
                <el-table-column prop="pick_count" label="货物种类"></el-table-column>
                <el-table-column prop="pick_count_num" label="总数"></el-table-column>
                <el-table-column prop="cut_status" label="状态">
                    <template scope="scope">
                        <span class="status-ball" :class="backClassName(scope.row.pick_status)"></span>
                        <span :class="scope.row.pick_status === 2 ? 'font-stop' : 'font-use'"
                        >{{backCutStatus(scope.row.pick_status)}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="操作" width="156">
                    <template slot-scope="scope">
                        <el-button
                            type="text"
                            class="marginRight32"
                            v-hasPermission="'order-delivery-list'"
    ...
    methods: {
        ...
        getListData() {
            ...
            let params = {
                ...
                pick_status_in: status_arr,     //不检索传 []
                sand_status_in: [],	    //不检索传 []
                cut_status_in: [],		//不检索传 []
            };
        ...
        handleView(index, row) {
            this.$router.push({path:'/order-delivery-list',query:{id:row.id.toString(),end_time:row.order_end_time}});
        },
